        ...
        if url.startswith("/api"):
            ...
            if url.startswith("/api/services"):
                ...
                if not config.conf().DEBUG:
                    ...
                    api_key = UserHelper.get_user_by_token(qs_dict["key"])
                    ...
                    user_info = model_to_dict(api_key)
                    # request.state.user = UserToken(**user_info)
                    request.state.user = user_info # todo: 유저 데이터 baseModel 만든 후 추후 변경
                    ...
                    if "authorization" in headers.keys():
                        ...
                        api_key_obj = UserHelper().get_user_by_token(key)
                        user_info = model_to_dict(api_key_obj)
